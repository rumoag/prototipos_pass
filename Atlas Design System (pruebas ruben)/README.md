...
### Itinerary day colors & map markers (restricted categorical palette)
A separate **12-family categorical palette** — *Índigo, Coral, Cian, Fucsia, Esmeralda, Ámbar, Lila, Naranja, Pizarra, Rosa, Cielo, Lima* (each a full 25→950 scale) — exists for **exactly two uses** and nothing else:
- **Itinerary day color.** Day N takes `--day-N` (the family's `500`); the sequence cycles after day 12. The day color fills the **map marker** and the **day dot/pill**, always with white content. Markers inherit the color of the day their place belongs to — re-assigning a place to another day recolors its pin. Selected marker: +20% size, 3px white halo + day-color ring, raised to `--z-raised`. Visited: drops to `neutral-400` + check.
- **Saved tags.** Each saved list picks one family — resting style `100` bg / `700` text / `500` icon; active (filter applied) uses the `500` fill with white content.

These families are **not** general UI colors: never use them for product text, states (use positive/negative/warning/informative), borders or actions. Brand identity stays Eucalipto + Terracota. See the *Itinerary day colors*, *Map markers* and *Saved tags* cards.
